...
export async function fetchRemoteBuffer(
  ...
  const timeoutHandle = setTimeout(() => {
    controller.abort(new FetchAbortedError('COLOR_EXTRACTOR_TIMEOUT', `Remote fetch exceeded ${timeoutMs}ms timeout.`))
  }, timeoutMs)

  let response: Response
  try {
    response = await fetcher(url, { signal: controller.signal, redirect: 'manual' })
  } catch (err) {
    ...
    if (err instanceof FetchAbortedError) {
      throw new ColorExtractorError(err.code, err.message, { cause: err })
    }
    throw new ColorExtractorError(
      'COLOR_EXTRACTOR_FETCH_FAILED',
  ...
  const contentType = headersToObject(response.headers)['content-type'] ?? null
  ...
  try {
    while (true) {
      ...
      if (received > maxBytes) {
        controller.abort(new FetchAbortedError(
          'COLOR_EXTRACTOR_INPUT_TOO_LARGE',
