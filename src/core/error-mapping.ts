...
function detectCode(
    message: string,
    fallback: ColorExtractorErrorCode,
): ColorExtractorErrorCode {
    if (matchAny(message, TIMEOUT_PATTERNS)) return 'COLOR_EXTRACTOR_TIMEOUT';
    if (matchAny(message, UNSAFE_URL_PATTERNS))
        return 'COLOR_EXTRACTOR_UNSAFE_URL';
    if (matchAny(message, FETCH_PATTERNS))
        return 'COLOR_EXTRACTOR_FETCH_FAILED';
    if (matchAny(message, DECODE_PATTERNS))
        return 'COLOR_EXTRACTOR_DECODE_FAILED';
    return fallback;
}
...
export function toColorExtractorError(
    ...
    const prefix = context.url
        ? `${context.operation} of ${context.url}`
        : context.operation;
