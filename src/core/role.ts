...
export function scorePrimary(
    ...
    switch (preset) {
        ...
        case 'balanced':
            return cluster.chroma ** 1.25 * Math.log(cluster.population + 1);
        case 'vibrant':
            return cluster.chroma ** 1.75 * Math.log(cluster.population + 1);
        case 'dominant':
...
export function scoreSecondary(
    ...
    const base =
        candidate.chroma *
        candidate.chroma *
        Math.log(candidate.population + 1);
    const weighted =
        base *
        hueWeight(primary, candidate) *
        contrastBoost(primary, candidate, options);
...
function rankBySecondaryScore(
    ...
    return candidates
        .map((c) => ({
            cluster: c,
            score: scoreSecondary(primary, c, options),
        }))
        .sort((a, b) => {
...
function clusterAsSecondaryColor(
    cluster: Cluster,
    source: ExtractedColor['source'],
): ExtractedColor {
    return {
...
export function selectSecondary(
    ...
    const { passing, rejected } = filterByContrastThreshold(
        primary,
        candidates,
        options,
    );
    ...
    if (passingRanked.length === 0) {
        if (options.secondary.fallback === 'harmony') {
            return {
                color: buildHarmonyFallback(primary, options),
...
export function applyLightnessGap(
    ...
    const adjustedChroma = Math.sqrt(
        adjustedLab.a * adjustedLab.a + adjustedLab.b * adjustedLab.b,
    );
...
export function buildPrimaryColor(
    cluster: Cluster,
): import('./types.js').ExtractedColor {
    return {
        hex: rgbToHex(cluster.rgb),
        rgb: cluster.rgb,
        hsl: cluster.hsl,
