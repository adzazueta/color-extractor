...
export function validateFilterCriteria(criteria: FilterCriteria): void {
  ...
  if (criteria.minBrightness > criteria.maxBrightness) {
    throw new ColorExtractorError(
      ...
      { cause: { minBrightness: criteria.minBrightness, maxBrightness: criteria.maxBrightness } },
    )
  }
}

function passesFilterUnvalidated(pixel: Pixel, criteria: FilterCriteria): boolean {
