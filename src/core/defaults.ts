import type { ExtractColorsOptions, PrimaryPreset, SecondaryFallbackMode, AnimatedHandling, SvgHandling } from './options.js'
...
const VALID_PRIMARY_PRESETS: readonly PrimaryPreset[] = ['strict', 'balanced', 'vibrant', 'dominant'] as const
const VALID_SECONDARY_FALLBACKS: readonly SecondaryFallbackMode[] = ['harmony', 'null', 'nearest'] as const
const VALID_ANIMATED: readonly AnimatedHandling[] = ['first-frame', 'all-frames', 'disabled'] as const
const VALID_SVG: readonly SvgHandling[] = ['disabled-in-node', 'enabled-in-node', 'disabled', 'enabled'] as const
...
function assertEnum<T extends string>(name: string, value: string, allowed: readonly T[]): void {
...
export function validateResolvedOptions(opts: ResolvedOptions): void {
  ...
  assertEnum('secondary.fallback', opts.secondary.fallback!, VALID_SECONDARY_FALLBACKS)
  assertNumber('secondary.contrastMinDE', opts.secondary.contrastMinDE!, 0)
  assertNumber('secondary.harmonyFallbackDeg', opts.secondary.harmonyFallbackDeg!, 0, 360)
...
export function resolveOptions(user?: ExtractColorsOptions): ResolvedOptions {
  const resolved = user === undefined ? deepClone(DEFAULT_OPTIONS) : deepMerge(DEFAULT_OPTIONS, user)
