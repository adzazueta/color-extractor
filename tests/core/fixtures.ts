...
function solidColor(r: number, g: number, b: number, a: number = 255): PixelInput {
...
export const FIXTURES = {
  ...
  mutedPlusVivid: (() => {
    const w = 16; const h = 16; const total = w * h
  ...
  mostlyTransparent: (() => {
    const w = 10; const h = 10; const total = w * h
    return makePixels(w, h, (i) =>
      i < total - 3 ? { r: 128, g: 128, b: 128 } : RED,
      0,
    )
  })(),

  rainbowPalette: (() => {
    const colors: RgbColor[] = [RED, GREEN, BLUE, YELLOW, PURPLE, ORANGE, TEAL, CRIMSON, WHITE, BLACK]
    const w = 50; const h = 50; const total = w * h
  ...
  multiColorPalette: (() => {
    ...
    const quadW = w / 2; const quadH = h / 2
    const colors: RgbColor[] = [RED, GREEN, BLUE, YELLOW]
    return makePixels(w, h, (i) => {
      const x = i % w; const y = Math.floor(i / w)
      const cx = Math.floor(x / quadW); const cy = Math.floor(y / quadH)
