...
import { ColorExtractorError, COLOR_EXTRACTOR_ERROR_CODES } from '../../src/core/index.js'
...
describe('typed errors with fixtures (ADZ-80)', () => {
  describe('AC: errors are instances of ColorExtractorError', () => {
    it('ColorExtractorError is a subclass of Error', () => {
      const err = new ColorExtractorError('COLOR_EXTRACTOR_UNSUPPORTED_INPUT', 'test')
    ...
    it('ColorExtractorError has code and cause properties', () => {
      const cause = { foo: 1 }
      const err = new ColorExtractorError('COLOR_EXTRACTOR_NO_VALID_PIXELS', 'msg', { cause })
  ...
  describe('AC: representative error paths are covered', () => {
    ...
    it('valid bicolorRedBlue fixture does NOT throw any error', () => {
      expect(() => runExtractionPipeline(FIXTURES.bicolorRedBlue)).not.toThrow()
    })
  })
})
