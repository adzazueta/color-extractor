...
describe('output flags through pipeline (ADZ-75)', () => {
  describe('AC: default result contains only required public color fields', () => {
    it('primary has hex, rgb, hsl but no lab or score', () => {
      ...
      expect((result.primary as unknown as Record<string, unknown>).lab).toBeUndefined()
      expect((result.primary as unknown as Record<string, unknown>).score).toBeUndefined()
    })

    it('palette and accents are absent by default', () => {
      const result = runExtractionPipeline(INPUT)
      expect((result as unknown as Record<string, unknown>).palette).toBeUndefined()
      expect((result as unknown as Record<string, unknown>).accents).toBeUndefined()
    })

    it('metadata is absent by default', () => {
      const result = runExtractionPipeline(INPUT)
      expect((result as unknown as Record<string, unknown>).metadata).toBeUndefined()
    })
  })
  ...
  describe('AC: optional fields are present when flags are enabled', () => {
    it('includePalette adds palette array', () => {
      ...
      expect((result as unknown as Record<string, unknown>).palette).toBeDefined()
      expect(Array.isArray((result as unknown as Record<string, unknown>).palette)).toBe(true)
    })

    it('includeAccents adds accents array', () => {
      const result = runExtractionPipeline(INPUT, { output: { includeAccents: true } })
      expect((result as unknown as Record<string, unknown>).accents).toBeDefined()
      expect(Array.isArray((result as unknown as Record<string, unknown>).accents)).toBe(true)
    })

    it('includeMetadata adds metadata object', () => {
      const result = runExtractionPipeline(INPUT, { output: { includeMetadata: true } })
      expect((result as unknown as Record<string, unknown>).metadata).toBeDefined()
    })

    it('includeLab adds lab to primary', () => {
      const result = runExtractionPipeline(INPUT, { output: { includeLab: true } })
      expect((result.primary as unknown as Record<string, unknown>).lab).toBeDefined()
    })

    it('includeScores adds score to primary', () => {
      const result = runExtractionPipeline(INPUT, { output: { includeScores: true } })
      expect((result.primary as unknown as Record<string, unknown>).score).toBeDefined()
    })
  })
  ...
  describe('AC: HSL can be excluded', () => {
    it('includeHsl: false removes hsl from primary', () => {
      const result = runExtractionPipeline(INPUT, { output: { includeHsl: false } })
      expect((result.primary as unknown as Record<string, unknown>).hsl).toBeUndefined()
    })
  })
})
