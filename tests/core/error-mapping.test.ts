...
import {
    detectErrorCode,
    toColorExtractorError,
} from '../../src/core/error-mapping.js';
...
describe('toColorExtractorError (ADZ-74)', () => {
    describe('AC: existing ColorExtractorError instances pass through unchanged', () => {
        it('returns the original error when input is already a ColorExtractorError', () => {
            const original = new ColorExtractorError(
                'COLOR_EXTRACTOR_TIMEOUT',
                'preset error',
            );
            const mapped = toColorExtractorError(original, {
                operation: 'fetch',
            });
    ...
    describe('AC: unknown errors fall back to the provided code', () => {
        ...
        it('honors a custom fallback', () => {
            ...
            const mapped = toColorExtractorError(
                err,
                { operation: 'x' },
    ...
    describe('AC: error message is actionable', () => {
        ...
        it('preserves the original cause', () => {
            ...
            const mapped = toColorExtractorError(cause, {
                operation: 'decode',
            });
    ...
    describe('AC: non-Error values are handled', () => {
        it('handles string errors', () => {
            const mapped = toColorExtractorError('just a string', {
                operation: 'decode',
            });
        ...
        it('handles undefined', () => {
            const mapped = toColorExtractorError(undefined, {
                operation: 'decode',
            });
            expect(mapped.message).toMatch(/unknown error/);
        });
...
describe('detectErrorCode (ADZ-74)', () => {
    ...
    it('returns the fallback for unknown errors', () => {
        ...
        ).toBe('COLOR_EXTRACTOR_IMAGE_TOO_LARGE');
    });

    it('uses the default fallback when none is given', () => {
        expect(detectErrorCode(new Error('???'))).toBe(
            'COLOR_EXTRACTOR_DECODE_FAILED',
        );
    });
});
