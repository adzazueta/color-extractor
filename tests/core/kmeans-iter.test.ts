...
describe('kmeans', () => {
        ...
        it('with iterations=0, centroids are the means of the initial assignment', () => {
            ...
            const means = [0, 1].map((ci) => {
                const members = samples.filter(
                    (_, i) => result.assignments[i] === ci,
                );
                const L =
                    members.reduce((a, s) => a + s.lab.L, 0) / members.length;
                const a =
                    members.reduce((a, s) => a + s.lab.a, 0) / members.length;
                const b =
                    members.reduce((a, s) => a + s.lab.b, 0) / members.length;
                return { L, a, b };
            });
            for (let i = 0; i < 2; i++) {
    ...
    describe('validation', () => {
        it('throws for clusters <= 0', () => {
            expect(() =>
                kmeans([sample(50, 0, 0, 0)], { clusters: 0, iterations: 1 }),
            ).toThrow(RangeError);
        });

        it('throws for negative clusters', () => {
            expect(() =>
                kmeans([sample(50, 0, 0, 0)], { clusters: -1, iterations: 1 }),
            ).toThrow(RangeError);
        });

        it('throws for clusters > samples.length', () => {
            expect(() =>
                kmeans([sample(50, 0, 0, 0), sample(60, 0, 0, 1)], {
                    clusters: 5,
                    iterations: 1,
                }),
            ).toThrow(RangeError);
