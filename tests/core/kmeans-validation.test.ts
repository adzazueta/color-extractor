...
describe('kmeans — validation (reviewer finding #1)', () => {
  describe('clusters parameter', () => {
    it('throws for NaN clusters', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: Number.NaN, iterations: 1 }))
        .toThrow(RangeError)
    })

    it('throws for Infinity clusters', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: Number.POSITIVE_INFINITY, iterations: 1 }))
        .toThrow(RangeError)
    })

    it('throws for fractional clusters', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: 1.5, iterations: 1 }))
        .toThrow(RangeError)
    })

    it('throws for zero clusters', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: 0, iterations: 1 }))
        .toThrow(RangeError)
    })

    it('throws for negative clusters', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: -1, iterations: 1 }))
        .toThrow(RangeError)
    })

    it('throws when clusters > samples.length', () => {
      expect(() => kmeans([sample(50, 0), sample(50, 1)], { clusters: 5, iterations: 1 }))
        .toThrow(RangeError)
    })
  })
  ...
  describe('iterations parameter', () => {
    it('throws for NaN iterations', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: 1, iterations: Number.NaN }))
        .toThrow(RangeError)
    })

    it('throws for Infinity iterations', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: 1, iterations: Number.POSITIVE_INFINITY }))
        .toThrow(RangeError)
    })

    it('throws for fractional iterations', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: 1, iterations: 1.5 }))
        .toThrow(RangeError)
    })
    ...
    it('accepts 0 iterations (initial state only)', () => {
      expect(() => kmeans([sample(50, 0)], { clusters: 1, iterations: 0 })).not.toThrow()
    })
  })
  ...
  describe('does not infinite loop', () => {
    it('terminates with Infinity iterations (rejected by validation)', () => {
      ...
      expect(() => kmeans(samples, { clusters: 2, iterations: Number.POSITIVE_INFINITY }))
        .toThrow(RangeError)
    })
  })
})
...
describe('initializeCentroids — validation', () => {
  it('throws for NaN k', () => {
    expect(() => initializeCentroids([sample(50, 0)], Number.NaN)).toThrow(RangeError)
  })

  it('throws for fractional k', () => {
    expect(() => initializeCentroids([sample(50, 0)], 1.5)).toThrow(RangeError)
  })
})
...
describe('kmeans — iterations count (exactly N recomputations)', () => {
  ...
  it('populations sum to total samples (no missing or double-counted)', () => {
    const samples = Array.from({ length: 20 }, (_, i) => sample(50 + (i % 7) * 5, i))
