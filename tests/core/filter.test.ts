...
describe('passesFilter', () => {
  ...
  describe('brightness (max channel)', () => {
    ...
    it('max channel above maxBrightness fails (near-white)', () => {
      expect(passesFilter(pixel(250, 200, 200, 255), CRITERIA)).toBe(false)
    })

    it('max channel at maxBrightness passes (boundary)', () => {
      expect(passesFilter(pixel(245, 100, 100, 255), CRITERIA)).toBe(true)
    })
  ...
  describe('saturation', () => {
    it('gray pixel fails (s=0 below minSaturation=8)', () => {
      expect(passesFilter(pixel(128, 128, 128, 255), CRITERIA)).toBe(false)
    })
    ...
    it('white fails (s=0)', () => {
      expect(passesFilter(pixel(240, 240, 240, 255), CRITERIA)).toBe(false)
    })
  ...
  describe('combined criteria', () => {
    ...
    it('brightness fails even if other criteria pass (max > 245)', () => {
      expect(passesFilter(pixel(255, 100, 100, 200), CRITERIA)).toBe(false)
    })

    it('saturation fails even if other criteria pass (vivid but mid-gray)', () => {
      expect(passesFilter(pixel(130, 128, 128, 200), CRITERIA)).toBe(false)
    })
  })
