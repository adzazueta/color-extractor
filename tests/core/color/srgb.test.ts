...
import { srgbToLinear, srgbByteToLinear } from '../../../src/core/color/srgb.js'
...
describe('srgbToLinear', () => {
  describe('boundary cases', () => {
    ...
    it('just above threshold uses the power branch', () => {
      ...
      const power = Math.pow((above + 0.055) / 1.055, 2.4)
...
describe('srgbByteToLinear', () => {
  ...
  describe('reference byte values', () => {
    ...
    it('byte 11 falls in power branch (11/255 > 0.04045)', () => {
      const power = Math.pow((11 / 255 + 0.055) / 1.055, 2.4)
    ...
    it('byte 128 (mid gray) matches reference', () => {
      expect(srgbByteToLinear(128)).toBeCloseTo(referenceEOTF(128 / 255), 12)
    })
  })
