...
function makeFull(overrides: Partial<FullExtractionResult> = {}): FullExtractionResult {
  return {
...
describe('applyOutputFlags', () => {
  ...
  describe('explicit includeScores: true', () => {
    it('includes all four score fields', () => {
      const result = applyOutputFlags(makeFull(), { includeScores: true })
  ...
  describe('includePalette', () => {
    it('includes palette array when true', () => {
      const result = applyOutputFlags(makeFull(), { includePalette: true })
    ...
    it('excludes palette when false', () => {
      const result = applyOutputFlags(makeFull(), { includePalette: false })
      expect(result.palette).toBeUndefined()
    })
  ...
  describe('includeAccents', () => {
    it('includes accents array when true', () => {
      const result = applyOutputFlags(makeFull(), { includeAccents: true })
      expect(result.accents).toHaveLength(1)
      expect(result.accents?.[0]?.hex).toBe('#aabbcc')
    })

    it('excludes accents when false', () => {
      const result = applyOutputFlags(makeFull(), { includeAccents: false })
      expect(result.accents).toBeUndefined()
    })
  })
  ...
  describe('includeMetadata', () => {
    it('includes metadata when true', () => {
      const result = applyOutputFlags(makeFull(), { includeMetadata: true })
    ...
    it('excludes metadata when false', () => {
      const result = applyOutputFlags(makeFull(), { includeMetadata: false })
      expect(result.metadata).toBeUndefined()
    })
  })
  ...
  describe('idempotence', () => {
    it('applying output flags twice produces the same result', () => {
      const full = makeFull()
      const once = applyOutputFlags(full, { includePalette: true, includeScores: true })
      const twice = applyOutputFlags(once as unknown as FullExtractionResult, {
        includePalette: true,
        includeScores: true,
      })
