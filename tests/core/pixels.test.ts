...
function expectUnsupportedInput(fn: () => unknown): void {
    try {
        ...
        expect((error as ColorExtractorError).code).toBe(
...
describe('normalizePixels', () => {
    ...
    describe('width validation', () => {
        it('throws for width = 0', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(0), 0, 1),
            );
        });

        it('throws for negative width', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(0), -1, 1),
            );
        });

        it('throws for non-integer width', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(0), 1.5, 1),
            );
        });

        it('throws for NaN width', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(0), Number.NaN, 1),
            );
        });

        it('throws for Infinity width', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(0), Number.POSITIVE_INFINITY, 1),
            );
        });
    });
    ...
    describe('height validation', () => {
        it('throws for height = 0', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(0), 1, 0),
            );
        });
        ...
        it('throws for non-integer height', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(0), 1, 2.5),
            );
        });
    });
    ...
    describe('channels validation', () => {
        it('throws for channels = 0', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(300), 10, 10, 0),
            );
        });

        it('throws for channels = 1 (grayscale not supported)', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(100), 10, 10, 1),
            );
        });

        it('throws for channels = 2', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(200), 10, 10, 2),
            );
        });

        it('throws for channels = 5', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(500), 10, 10, 5),
            );
        });
    });
    ...
    describe('data length validation', () => {
        ...
        it('throws for data too long (RGBA)', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(404), 10, 10),
            );
        });

        it('throws for data too short (RGB)', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(30), 10, 10, 3),
            );
        });

        it('throws for 3-channel data with 4-channel size', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(400), 10, 10, 3),
            );
        });

        it('throws for 4-channel data with 3-channel size', () => {
            expectUnsupportedInput(() =>
                normalizePixels(new Uint8Array(300), 10, 10, 4),
            );
        });
    ...
    describe('core API surface', () => {
        it('no DOM or Node API dependencies', async () => {
            ...
            const src = await fs.readFile(
                new URL('../../src/core/pixels.ts', import.meta.url),
                'utf-8',
            );
