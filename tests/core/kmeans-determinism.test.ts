...
import { kmeans, buildClusters, initializeCentroids } from '../../src/core/kmeans.js'
import { normalizePixels } from '../../src/core/pixels.js'
import { convertRgbSamplesToLab, sampleSquareGrid } from '../../src/core/sample.js'
import { FIXTURES } from './fixtures.js'

function runPipeline(fixtureKey: keyof typeof FIXTURES, k: number = 5) {
  const fixture = FIXTURES[fixtureKey]
  const normalized = normalizePixels(fixture.data as Uint8Array, fixture.width, fixture.height)
...
describe('deterministic K-means (ADZ-87)', () => {
  describe('AC: same pixels produce same clusters', () => {
    it('bicolorRedBlue fixture is deterministic across two runs', () => {
      ...
      expect(a.kmeansResult.populations).toEqual(b.kmeansResult.populations)
      expect(a.kmeansResult.assignments).toEqual(b.kmeansResult.assignments)
    })
  ...
  describe('empty cluster handling with fixtures', () => {
    it('does not crash when given a monochrome fixture with many clusters', () => {
      ...
      const sampled = sampleSquareGrid(normalized, 150)
      const labs = convertRgbSamplesToLab(sampled)
      expect(() => kmeans(labs, { clusters: 10, iterations: 7 })).not.toThrow()
