...
describe('parseLocationHeader (ADZ-57)', () => {
    it('returns absolute URLs as-is', () => {
        expect(
            parseLocationHeader('https://x.com/y', 'https://base.com/'),
        ).toBe('https://x.com/y');
    });

    it('resolves relative URLs against the base', () => {
        expect(parseLocationHeader('/y', 'https://base.com/a/b')).toBe(
            'https://base.com/y',
        );
    });
    ...
    it('returns null for unparseable values', () => {
        expect(
            parseLocationHeader('http://[invalid', 'https://base.com'),
        ).toBeNull();
    });
});
...
describe('followRedirects (ADZ-57)', () => {
        ...
        it('throws COLOR_EXTRACTOR_UNSAFE_URL after maxRedirects hops', async () => {
            ...
            await expectUnsafe(
                followRedirects('https://a.com/', {
                    resolveAndFetch: routeResolveAndFetch(map),
                    maxRedirects: 2,
                }),
            );
        });
    ...
    describe('AC: redirects to disallowed protocols fail', () => {
        it('throws when a redirect points to file://', async () => {
            ...
            await expectUnsafe(
                followRedirects('https://a.com/', { resolveAndFetch }),
            );
        });
    ...
    describe('AC: review #4 — body is cancelled on redirect overshoot, bad status, and intermediate hops', () => {
        ...
        it('cancels the response body when maxRedirects is exceeded', async () => {
            ...
            const resolveAndFetch: ResolveAndFetch = async (url) => {
                if (url === 'https://a.com/') {
                    return new Response(hop1.stream, {
                        status: 302,
                        headers: { location: 'https://b.com/' },
                    });
                }
                return new Response(Buffer.from(new Uint8Array(1)), {
                    status: 200,
                });
            };
            let threw = false;
        ...
        it('cancels intermediate body on multi-hop redirect chain (Finding #4)', async () => {
            ...
            const resolveAndFetch: ResolveAndFetch = async (url) => {
                ...
                if (url === 'https://b.com/') {
                    return new Response(hop2.stream, {
                        status: 302,
    ...
    describe('AC: resolveAndFetch defaults to defaultResolveAndFetch', () => {
        it('does not throw DECODE_FAILED when resolveAndFetch is omitted', async () => {
            ...
            try {
                ...
                threw = true;
                expect((e as ColorExtractorError).code).not.toBe(
