...
describe('validateContentType (ADZ-63)', () => {
    describe('AC: non-image content types are rejected when validation is enabled', () => {
        it('rejects text/html', () => {
            expect(() => validateContentType('text/html', opts())).toThrow(
                ColorExtractorError,
            );
            try {
        ...
        it('rejects application/json', () => {
            expect(() =>
                validateContentType('application/json', opts()),
            ).toThrow(ColorExtractorError);
        });

        it('rejects text/plain', () => {
            expect(() => validateContentType('text/plain', opts())).toThrow(
                ColorExtractorError,
            );
        });
    });
    ...
    describe('AC: allowed image types are accepted', () => {
        it('accepts image/jpeg', () => {
            expect(() =>
                validateContentType('image/jpeg', opts()),
            ).not.toThrow();
        });

        it('accepts image/png', () => {
            expect(() =>
                validateContentType('image/png', opts()),
            ).not.toThrow();
        });

        it('accepts image/webp', () => {
            expect(() =>
                validateContentType('image/webp', opts()),
            ).not.toThrow();
        });

        it('accepts image/gif', () => {
            expect(() =>
                validateContentType('image/gif', opts()),
            ).not.toThrow();
        });

        it('accepts image/avif', () => {
            expect(() =>
                validateContentType('image/avif', opts()),
            ).not.toThrow();
        });
    });
    ...
    describe('AC: content type with charset is normalized', () => {
        it('accepts image/png; charset=utf-8', () => {
            expect(() =>
                validateContentType('image/png; charset=utf-8', opts()),
            ).not.toThrow();
        });

        it('rejects text/html; charset=utf-8', () => {
            expect(() =>
                validateContentType('text/html; charset=utf-8', opts()),
            ).toThrow(ColorExtractorError);
        });
    });
    ...
    describe('AC: SVG content type follows Node SVG policy', () => {
        ...
        it('allows image/svg+xml when svg is enabled', () => {
            expect(() =>
                validateContentType('image/svg+xml', opts({ svg: 'enabled' })),
            ).not.toThrow();
        });
    });
});
