...
describe('validateRemoteProtocol (ADZ-62)', () => {
  ...
  describe('AC: unsupported protocols are rejected with COLOR_EXTRACTOR_UNSAFE_URL', () => {
    ...
    it('rejects ftp://', () => {
      expectUnsafeUrl(() => validateRemoteProtocol('ftp://example.com/image.png'))
    })

    it('rejects data: URLs', () => {
      expectUnsafeUrl(() => validateRemoteProtocol('data:image/png;base64,AAAA'))
    })

    it('rejects javascript: URLs', () => {
      expectUnsafeUrl(() => validateRemoteProtocol('javascript:alert(1)'))
    })
  })

  describe('AC: custom allowedProtocols is honored', () => {
    it('accepts only https when configured', () => {
      expectUnsafeUrl(() => validateRemoteProtocol('http://x.com', { allowedProtocols: ['https:'] }))
      expect(validateRemoteProtocol('https://x.com', { allowedProtocols: ['https:'] })).toBeDefined()
    })

    it('accepts multiple custom protocols', () => {
      const r = validateRemoteProtocol('ws://x.com', { allowedProtocols: ['http:', 'https:', 'ws:'] })
  ...
  describe('AC: empty allowedProtocols list is rejected', () => {
    it('throws when no protocols are allowed', () => {
      expectUnsafeUrl(() => validateRemoteProtocol('https://x.com', { allowedProtocols: [] }))
    })
  })
  ...
  describe('AC: error message names the offending protocol and the allowed list', () => {
    it('mentions both the rejected protocol and the allowed list', () => {
      try {
        validateRemoteProtocol('file:///etc/passwd', { allowedProtocols: ['http:', 'https:'] })
        expect.fail('expected throw')
      } catch (e) {
...
describe('assertAllowedProtocol (ADZ-62)', () => {
  it('returns the parsed URL for allowed protocols', () => {
    const r = assertAllowedProtocol('https://example.com:8080/a/b.png', ['https:'])
    expect(r.protocol).toBe('https:')
