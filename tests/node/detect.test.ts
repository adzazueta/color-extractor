...
describe('detectNodeInputKind (ADZ-56)', () => {
  ...
  describe('AC: http(s) string is treated as a remote URL', () => {
    it('returns "remoteUrl" for https://', () => {
      expect(detectNodeInputKind('https://example.com/image.png')).toBe('remoteUrl')
    })

    it('returns "remoteUrl" for http://', () => {
      expect(detectNodeInputKind('http://example.com/image.png')).toBe('remoteUrl')
    })
  })
  ...
  describe('AC: non-http strings are treated as local paths', () => {
    ...
    it('returns "localPath" for a Windows-style path', () => {
      expect(detectNodeInputKind('C:\\images\\photo.jpg')).toBe('localPath')
    })
  })
  ...
  describe('AC: unsupported inputs are rejected', () => {
    ...
    it('returns "unsupported" for a plain object', () => {
      expect(detectNodeInputKind({ data: Buffer.alloc(1) })).toBe('unsupported')
    })
  ...
  describe('AC: assertSupportedNodeInput throws ColorExtractorError', () => {
    it('does not throw for supported inputs', () => {
      expect(() => assertSupportedNodeInput(Buffer.alloc(1))).not.toThrow()
      expect(() => assertSupportedNodeInput('https://x.com/y.png')).not.toThrow()
    })

    it('throws for null', () => {
      expect(() => assertSupportedNodeInput(null)).toThrow(ColorExtractorError)
    })

    it('throws for unsupported types', () => {
      expect(() => assertSupportedNodeInput(42)).toThrow(ColorExtractorError)
    })
