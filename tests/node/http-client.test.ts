...
import {
    createResolveAndFetch,
    type LookupFunction,
} from '../../src/node/http-client.js';

function privateLookup(addr: string): LookupFunction {
    return async (_hostname) => [{ address: addr, family: 4 as const }];
}

const _publicLookup: LookupFunction = async (_hostname) => [
