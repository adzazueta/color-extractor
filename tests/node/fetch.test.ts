...
import {
    type Fetcher,
    fetchRemoteBuffer,
    type RemoteBufferResult,
} from '../../src/node/fetch.js';
...
function okFetch(
    body: Uint8Array,
    init: { contentLength?: number; contentType?: string } = {},
): Fetcher {
...
describe('fetchRemoteBuffer (ADZ-58)', () => {
    describe('AC: returns the response body and content type on success', () => {
        ...
        it('returns the content-type header when present', async () => {
            ...
            const result = await fetchRemoteBuffer(
                'https://x.com/y.jpg',
                {},
                okFetch(body, { contentType: 'image/jpeg' }),
            );
    ...
    describe('AC: review #4 — response body is cancelled on non-OK, redirect, and oversize', () => {
        ...
        ): { fetcher: Fetcher; isCancelled: () => boolean } {
            let cancelled = false;
            const stream = new ReadableStream<Uint8Array>({
                start(controller) {
        ...
        it('cancels body on 302 redirect', async () => {
            const { fetcher, isCancelled } = makeStreamingFetcher(302, {
        ...
        it('cancels body when Content-Length exceeds maxBytes', async () => {
            const { fetcher, isCancelled } = makeStreamingFetcher(200, {
                'content-length': '9999',
            });
            let threw = false;
            try {
...
describe('RemoteBufferResult (ADZ-58)', () => {
    it('concatenates streamed chunks in order', async () => {
        ...
        const result: RemoteBufferResult = await fetchRemoteBuffer(
            'https://x.com/y',
            {},
            fetcher,
        );
