...
describe('detectBrowserInputKind (ADZ-53)', () => {
  ...
  describe('AC: URL string is detected as url', () => {
    it('returns "url" for an http URL', () => {
      expect(detectBrowserInputKind('http://example.com/image.png')).toBe('url')
    })
  ...
  describe('AC: unsupported inputs are rejected', () => {
    ...
    it('returns "unsupported" for a Buffer (Node-only type)', () => {
      expect(detectBrowserInputKind(Buffer.from([1]))).toBe('unsupported')
    })
  ...
  describe('AC: assertSupportedBrowserInput throws ColorExtractorError', () => {
    it('does not throw for supported inputs', () => {
      expect(() => assertSupportedBrowserInput('https://x.com/y.png')).not.toThrow()
      expect(() => assertSupportedBrowserInput(new Blob(['x']))).not.toThrow()
    })

    it('throws for null', () => {
      expect(() => assertSupportedBrowserInput(null)).toThrow(ColorExtractorError)
    })

    it('throws for unsupported types', () => {
      expect(() => assertSupportedBrowserInput(42)).toThrow(ColorExtractorError)
    })
