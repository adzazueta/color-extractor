...
function expectCondition(entry: ConditionalExport, condition: string): string {
  ...
  expect(typeof value, `condition "${condition}" must be a string`).toBe('string')
...
function readExport(subpath: string): ConditionalExport {
  ...
  expect(typeof entry, `subpath "${subpath}" must be an object with conditions`).toBe('object')
...
describe('public exports contract', () => {
  ...
  describe('root . entrypoint', () => {
    it('declares types, browser, node and default conditions', () => {
      ...
      expect(Object.keys(root).sort()).toEqual(['browser', 'default', 'node', 'types'])
    })
  ...
  describe('runtime subpaths', () => {
    it.each(['./browser', './node', './core'])('%s is defined', (subpath) => {
      expect(pkg.exports[subpath]).toBeDefined()
    })

    it.each(['./browser', './node', './core'])('%s points to an existing types declaration', (subpath) => {
      const types = expectCondition(readExport(subpath), 'types')
      expect(types).toMatch(/\.d\.ts$/)
      expect(existsSync(resolve(rootDir, types))).toBe(true)
    })

    it.each(['./browser', './node', './core'])('%s points to an existing import bundle', (subpath) => {
      const importPath = expectCondition(readExport(subpath), 'import')
